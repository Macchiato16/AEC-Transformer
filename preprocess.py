...
def _prepare_features(farend, mic, target_sr, frame_length, hop_length, win_length, target=None):
    ...
    mic_amp, mic_stft_phase = _amplitude_phase(mic_stft)  # mic_amp: (n_fft // 2 + 1, n_frames), mic_stft_phase: (n_fft // 2 + 1, n_frames)
    if target is not None:
    ...
    # 转置 mic_stft_phase 和 target_amp (如果存在)
    mic_stft_phase = mic_stft_phase.T  # (n_frames, 频率维度)
    if target is not None:
        target_amp = target_amp.T  # (n_frames, 频率维度)
        return features, mic_stft_phase, target_amp
    else:
        return features, mic_stft_phase  # 只返回 features 和转置后的 mic_stft_phase
...
def preprocess_audio(farend_path, mic_path, target_path, target_sr=16000,
    ...
    """
    features, mic_stft_phase, target_amp = _prepare_features(
        farend_path, mic_path, target_sr, frame_length, hop_length, win_length, target=target_path
    )

    # 7. 计算理想掩码 (IRM)
    target_mask = np.clip(target_amp / (np.abs(mic_stft_phase) + 1e-10), 0.0, 1.0) # target_mask: (n_frames, n_fft // 2 + 1)
    num_frames = features.shape[0]

    return features, target_mask, num_frames, mic_stft_phase
...
def prepare_features_for_inference(farend_path, mic_path, seq_len, target_sr=16000,
    ...
    为模型推理准备输入特征。
    """
    features, mic_phases = _prepare_features(
    ...
    while start_frame + seq_len <= num_frames:
        ...
        sub_phase = mic_phases[start_frame:end_frame, :]   # sub_phase: (seq_len, n_fft // 2 + 1)
    ...
    if start_frame < num_frames:
        ...
        sub_phase = mic_phases[start_frame:end_frame, :]  # sub_phase: (last_seq_len, n_fft // 2 + 1)
    ...
    return features_tensor, mic_phases_tensor, num_frames, original_lengths
...
if __name__ == "__main__":
    ...
    try:
        features, target_mask, num_frames, mic_stft_phase = preprocess_audio(
        ...
        print("Mic STFT phase shape:", mic_stft_phase.shape)
        ...
        assert np.isfinite(mic_stft_phase).all(), "Mic STFT phase contains NaN or Inf values"
    ...
    try:
        features_tensor, mic_phases_tensor, num_frames, original_lengths = prepare_features_for_inference(
